...
public class Play extends ElementObj{
    ...
    private String fx = "up"; //记录当前面向的方向,默认为up
...
/*    public Play(int x, int y, int w, int h, ImageIcon icon) {
    ...
    public void keyClick(boolean bl,int key){
        if(bl){
            switch(key){
                ...
                case 40: this.up = false;this.left = false;this.right=false;
                         this.down = true;
                        if(this.isHitable()){//普通模式下
                            this.fx = "down"; break;
                        }else{
                ...
                case 32: this.pkType = true; break; //按下空格发射子弹
            }
        }else{
            switch(key){
    ...
    * */
    ...
    public String toString(){
        ...
        switch(this.fx){ //普通状态贴图和无敌状态贴图均需要考虑
            case "up": BulletX = this.getX() + (tankImageSize/2); break;
            case "ultimate_up": BulletX = this.getX() + (tankImageSize/2); break;
            case "down": BulletX = this.getX() + (tankImageSize/2); BulletY = this.getY() + (tankImageSize/2); break;
            case "ultimate_down": BulletX = this.getX() + (tankImageSize/2); BulletY = this.getY() + (tankImageSize/2); break;
            case "left":BulletY = this.getY() + (tankImageSize/2); break;
            case "ultimate_left":BulletY = this.getY() + (tankImageSize/2); break;
            case "right":BulletX = this.getX() + (tankImageSize);BulletY = this.getY()+(tankImageSize/2);break;
    ...
    public void handleStarPK(long gameTime) {
        ...
        //System.out.println(getStautsChangedTime());
        this.setHitable(false);//设置玩家为不可击中状态
        System.out.println("玩家1无敌状态启动");
    }
    ...
    * */
    ...
    public void handleHealPK() {
        ...
        //System.out.println("玩家生命值加一，目前为:" + this.getHp());
    }
}
