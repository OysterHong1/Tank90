...
public class FastEnemy extends ElementObj{
    ...
    private String fx = "fast_enemy_down";

    private int botTankSpeed = 3; //快速坦克速度为3
    ...
    @Override //敌人射击，子弹添加函数,快速敌人射击概率为2%
    public void add(long gameTime) {
        if(firecontrolNumber > 55 || gameTime - filetime < 50){return;}
        filetime = gameTime;
    ...
    public String toString(){
        ...
        switch(this.fx){
            case "fast_enemy_up": BulletX = this.getX() + (tankImageSize/2); break;
            case "fast_enemy_down": BulletX = this.getX() + (tankImageSize/2); BulletY = this.getY() + (tankImageSize/2); break;
            case "fast_enemy_left":BulletY = this.getY() + (tankImageSize/2); break;
            case "fast_enemy_right":BulletX = this.getX() + (tankImageSize);BulletY = this.getY()+(tankImageSize/2);break;
        }
